...
![Redis-Cluster集群模式](http://sunyanping.gitee.io/it-keep/ASSET//Redis-Cluster集群模式.png)
...
## 集群的扩容（集群节点添加/删除）
**一般情况下，Redis集群只用创建一次即可，集群创建好了之后，可以在不影响集群的正常状态动态的添加/删除节点。**        
参考文章：[Redis集群增加节点和删除节点](https://www.cnblogs.com/hopeofthevillage/p/11535683.htm)      

**集群扩容：**    
1. 准备新的Redis节点。一般复制redis.conf，然后修改里面的端口等信息。
2. 使用`redis-server`命令启动准备的节点
3. 将新增的节点添加到集群中，作为其中的一个主节点。`redis-trib.rb add-node <ip:port>  <ip:port>`，第一个是将要新增的节点，第二个是集群中原有的任意一个主节点。    
*添加完之后，新的节点中没有数据，也没有分配hash槽，所以这个节点是不会被选中的。*
4. 使用 redis-trib 程序， 将集群中的某些hash槽移动到新节点里面， 新节点就会成为真正的主节点了。执行命令`redis-trib.rb reshard <ip:port>`（节点是集群中任意一个主节点），然后输入要分配的hash槽数量。一般是均分，可以根据hash槽总数量和节点数计算一下。    
*在执行命令时，其中要输入分配的hash槽、从哪个节点分配（all,done）、新的节点的ID等信息。*
5. 为主节点添加从节点，先将节点加如集群。`redis-trib.rb add-node <ip:port>  <ip:port>`，第一个是将要新增的节点，第二个是集群中原有的任意一个节点。
6. 将节点设为某个节点的从节点。先登陆到新增的节点`redis-cli -p <port> -h <ip>`，再执行命令`cluster replicate <node_ID>`（node_ID是主节点的ID）。即可。
